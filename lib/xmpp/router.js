 ...
 */
DomainContext.prototype.send = function(stanza, from) {
    if (stanza.root)
        stanza = stanza.root();
    ...
    if (!stanza.attrs.to) {
        ...
        stanza.c('error', { type: 'modify' }).c('jid-malformed', { xmlns: NS_XMPP_STANZAS });
        this.receive(stanza, from);
...
DomainContext.prototype.receive = function(stanza, from) {
    if (this.stanzaListener)
        this.stanzaListener(stanza, from);
};
...
Router.prototype.send = function(stanza, from) {
    if (stanza.root)
        stanza = stanza.root();
    ...
    if (toDomain && this.hasContext(toDomain)) {
        // inner routing
        this.getContext(toDomain).receive(stanza, from);
