...
function C2S(options) {
    ...
    if (options.tls)
	this.credentials = { key: fs.readFileSync(options.tls.keyPath, 'ascii'),
			     cert: fs.readFileSync(options.tls.certPath, 'ascii')
			   };
    ...
    self.router.register(options.domain, function(stanza, from) {
        self.route(stanza, from);
    });
}
...
/**
* C2S Router */
C2S.prototype.route = function(stanza, from) {
    var self = this;
    if(stanza.attrs && stanza.attrs.to) {
        ...
        if(self.sessions.hasOwnProperty(toJid.bare().toString())) {
            ...
            // The user is offline
            from.emit('recipient_offline', stanza);
        }
    }
    else {
...
C2SClient.prototype.onRawStanza = function(stanza) {
    ...
    stanza.attrs.to != this.jid.bare().toString()) {
        this.c2s.router.send(stanza, this);
    }
    else {
